...
- 在隔离 Hyper-V guest 中运行真实输入，不占用用户前台；所有 worker 合计限制为 10 个逻辑处理器/20 GiB，并以多个隔离桌面并行。建立 source-to-scenario 影响图和内容寻址证据，只重跑受影响、过期和不可缓存测试，同时持续执行复杂/大文档真实构建与长期 soak。
