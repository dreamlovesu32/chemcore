...
test("saved-document node oracle kills swapped and display-only carbon-label visibility mutants", () => {
  const bytes = Buffer.from(JSON.stringify({ resources: { mol: { type: "molecule_fragment2d", data: { nodes: [
    { id: "n_1", element: "C", atomicNumber: 6, charge: 0, numHydrogens: 3, atomProperties: { showTerminalCarbonLabel: true }, label: { text: "H3C", sourceText: "CH3" } },
    { id: "n_2", element: "C", atomicNumber: 6, charge: 0, numHydrogens: 2, atomProperties: { showNonTerminalCarbonLabel: true }, label: { text: "H2\nC", sourceText: "CH2" } },
  ] } } } }));
  const expected = [
    ...
    { id: "n_2", element: "C", atomicNumber: 6, charge: 0, numHydrogens: 2, showTerminalCarbonLabel: null, showNonTerminalCarbonLabel: true, labelText: "H2\nC", labelSourceText: "CH2" },
  ];
  assert.equal(evaluateDocumentNodeProperties(bytes, expected).passed, true);
