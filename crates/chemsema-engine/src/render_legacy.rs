...
fn legacy_bond_stroke_width(document: &ChemSemaDocument, object: &SceneObject) -> f64 {
    object
        .style_ref
        .as_ref()
...
fn legacy_label_font_family(document: &ChemSemaDocument, object: &SceneObject) -> String {
    object
        .style_ref
        .as_ref()
        .and_then(|style_ref| document.styles.get(style_ref))
...
fn legacy_label_font_size(document: &ChemSemaDocument, object: &SceneObject) -> f64 {
    object
        .style_ref
        .as_ref()
        .and_then(|style_ref| document.styles.get(style_ref))
