use crate::{ChemSemaDocument, MoleculeFragment, Node, Point, SceneObject};
...
pub fn refresh_repeating_units(document: &mut ChemSemaDocument) -> bool {
...
fn detect_repeating_units(document: &ChemSemaDocument) -> Vec<RepeatingUnitPlan> {
...
fn bracket_candidates(document: &ChemSemaDocument) -> Vec<BracketCandidate> {
    document
        .scene_objects()
        .into_iter()
...
fn build_repeating_unit_expansion(
    ...
    Some(json!({
        "schema": "chemsema.repeatingUnitExpansion.v1",
        "complete": true,
        "count": repeat_count,
        "atoms": atoms,
        "bonds": bonds,
...
fn repeating_unit_json(plan: &RepeatingUnitPlan) -> Value {
    json!({
        "schema": "chemsema.repeatingUnit.v1",
        "id": plan.unit_id,
        "kind": "multiple-group",
