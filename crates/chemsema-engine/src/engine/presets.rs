...
use crate::{
    render_document, render_primitives_bounds, Bond, ChemSemaDocument, EditorOptions,
...
pub(super) fn document_style_preset_from_document(document: &ChemSemaDocument) -> &'static str {
...
pub(crate) fn editor_options_from_document(document: &ChemSemaDocument) -> EditorOptions {
...
fn infer_cdxml_document_bond_metrics(document: &ChemSemaDocument) -> Option<InferredBondMetrics> {
...
fn document_content_center(document: &ChemSemaDocument) -> Option<Point> {
...
fn scale_document_for_style_preset(document: &mut ChemSemaDocument, factor: f64, anchor: Point) {
    ...
    if let Ok(mut next_document) = serde_json::from_value::<ChemSemaDocument>(value) {
...
fn apply_existing_document_style_preset(document: &mut ChemSemaDocument, options: &EditorOptions) {
...
fn update_document_style_info_defaults(
    ...
    document.style.defaults = BTreeMap::from([
        ...
        ("chainAngle".to_string(), chain_angle),
        (
            "lineWidth".to_string(),
