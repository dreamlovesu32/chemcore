...
impl Engine {
    pub fn text_format_icon_svg(kind: &str) -> String {
        ...
        let class_name = if kind == "tool" {
            "chemsema-icon cc-tool-icon cc-text-tool-icon"
        } else if matches!(kind, "chemical" | "subscript" | "superscript") {
            "chemsema-icon cc-text-format-icon cc-script-icon"
        } else {
            "chemsema-icon cc-text-format-icon"
        };
    ...
    pub fn bond_tool_icon_svg(variant: BondVariant, stroke_width: f64, bold_width: f64) -> String {
        ...
        crate::primitives_to_svg_viewbox(
            &primitives,
            [0.0, 0.0, 24.0, 24.0],
            Some("chemsema-icon cc-bond-icon"),
        )
        .replace("#000000", "currentColor")
    }
    ...
    pub(super) fn preview_bond_overlay_document(&self) -> Option<ChemSemaDocument> {
