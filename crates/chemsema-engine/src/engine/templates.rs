...
use crate::{
    ...
    ChemSemaDocument, DoubleBond, DoubleBondPlacement, EndpointHit, Node, Point, PointerEvent,
...
impl Engine {
    ...
    pub(super) fn template_preview_document(&self) -> Option<ChemSemaDocument> {
    ...
    pub(super) fn template_preview_overlay_document(&self) -> Option<ChemSemaDocument> {
        let drag = self.template_drag.as_ref()?;
        if !drag.has_dragged {
            return None;
        }
...
fn endpoint_click_ring_axis_angle(document: &ChemSemaDocument, anchor: &BondAnchor) -> f64 {
