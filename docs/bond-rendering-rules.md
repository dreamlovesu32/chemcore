...
- A wavy bond is a sequence of fixed-size quarter-circle cubic segments. Its half-wave step equals its amplitude, and the number of complete half waves is `max(4, floor(length / amplitude))`; ChemDraw leaves any remainder shorter than one half wave undrawn instead of stretching the pattern to the endpoint. Consequently, source lengths immediately below and above an integer multiple can intentionally end at different phases. Every Bezier handle is `0.5522847498307936 * amplitude` along the local tangent. Do not redistribute the remainder across the waves, subtract an extra amplitude, or cap the quarter-circle handle to half the axial step.
