...
23. 自动化学标签的连接方向按 `15°` 坐标轴扇区分类，而不是按归一化向量分量是否大于零。右扇区键与非左扇区连接组合时保持反向横排；全部连接进入下方或上方扇区后才切换为上下堆叠。`14.9°/15.0°` 精扫、8/10/14 pt 字号及 10/14.4/24 pt 键长交叉验证了固定边界；短键在源坐标保留位数不足时按量化后的实际角度判定，不能按请求角度或文件编号分支。
