...
| GUI 测试平台 | 实施中/正式 production black-box sentinel 已通过 | 版本化 Schema/runner、真实 Playwright 路径、覆盖/影响/资源门禁、无人值守 Hyper-V 登录、逐项验证的专用用户 baseline、内容寻址候选部署、受守卫的 UIA/CDP 定位、带 SHA 验证的 guest 真实点击/拖拽，以及不可变报告对象和经过验证的 manifest 已运行；完整截图/trace/log bundle、确定性重置、低延迟持久输入、完整 capability 矩阵、复杂与大文档构建、模型/故障/变异测试和展示资格仍待完成 |
