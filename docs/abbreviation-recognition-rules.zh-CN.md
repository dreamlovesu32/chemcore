...
多连接标签使用 ChemDraw 实测的“最大开放扇区”规则。先按文件中实际坐标得到连接
角度并排序，找到相邻连接之间最大的角间隙，再用其补弧（连接占据的角范围）的中线
决定文字流。反向横排与正向横排各占 `135°`：前者为 `292.5°..67.5°`，后者为
`112.5°..247.5°`；中间两个 `45°` 扇区分别产生上方和下方堆叠。间隙差不超过
`0.001°` 时进入明确的对称分支：先选择中心落在水平正轴的开放间隙，否则选择最靠近
正上方的间隙，距离完全相同时取顺时针一侧。两根键严格相反时按无向轴的
`22.5°/90°/157.5°` 边界处理。三根键的实际角间隙都处于 `120°±3°` 内时，
继续拟合共同的 `120°` 相位；静默探针在 8/10/14 pt 标签及 10/14.35/24 pt
键长下都把切换点定位在 `2.95°` 与 `3.05°` 之间。超出该固定窗口后，唯一最大
间隙才生效。ChemDraw SVG/CDXML 探针覆盖了二、三、四连接的完整 `30°` 网格（共 1,079 例）、
边界精扫、多种字号和多种键长，分析器误差为零。规则只使用量化后的实际坐标，不使用
文件名或请求角度。单连接末端标签仍使用完整左右半平面；只有近似垂直的单连接才交给
碰撞规则决定方向。
