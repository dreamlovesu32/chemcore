...
### Experiment: 窄命中 packaged point-style（多行 + centered + mixed-script）

- Hypothesis:
  - 既然 harness 已经证明：
    - `large font + rect-style` 会吞掉 fallback `" "`
    - `large font + point-style` 能保住 fallback `" "`
  - 那么在产品代码里，只对：
    - packaged
    - 多行 text object
    - centered
    - mixed-script 行
    切到 point-style `DrawString`，应该能恢复标题第二行的 fallback 空格
- Code path touched:
  - `apps/chemcore-office/src/windows_office/emf_preview/renderer.rs`
  - 在 `draw_gdiplus_text()` 按行决定 `point_style_line`
  - `draw_gdiplus_text_run()` 在命中行改用 `RectF { Width: 0, Height: 0 }`
- Fixtures used:
  - `tmp/title-only.payload.json`
  - `tmp/subset-9.payload.json`
  - `tmp/thiocyanation-source.cdxml`
- Expected result:
  ...
  - 且整体像素不应明显劣化
- Actual result:
  - 第一版没有真正命中，因为 payload 里的脚本标记是：
    - `subscript / superscript`
    - 不是 `sub / super`
  - 修正命中条件后：
    - `title-only` 恢复了 fallback `" "`
    - `subset-9` 也恢复了 fallback `" "`
    - 大文件标题第二行同样恢复了 fallback `" "`
  - 但全图 top-left 逐像素重叠反而变差：
    - baseline `IoU ≈ 0.7505`
    - experiment `IoU ≈ 0.7329`
- Relevant files:
  - `tmp/pointstyle-exp/title-only-current.emf`
  - `tmp/pointstyle-exp/subset-9-current.emf`
  - `tmp/pointstyle-exp/thiocyanation.emf`
  - `tmp/pointstyle-exp/render/metrics-top-left.json`
- Kept or reverted:
  - 计划回退产品代码
- Conclusion:
  - “point-style 能把空格救回来”这件事在产品路径里成立
  - 但当前命中范围太宽，会拖坏别的文字行
  - 下一步应继续缩窄到：
    - **只有 subscript 结束后，紧跟 leading-space normal run 的那一行**
