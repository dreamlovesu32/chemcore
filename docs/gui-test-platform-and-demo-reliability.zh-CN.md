...
production 制品导出现在已经投入运行并且 fail-closed。runner 在 `chemsema.gui.run.v1` 中要求 driver payload 描述符，把每个 SHA-256 纳入 evidence key，并把报告与全部 payload 保存为不可变内容寻址对象，再由经过 Schema 验证的 manifest 引用。production driver 在 guest 内采集最终 PNG、完整 DOM、当前 CCJS JSON、结构化 engine/window/render 状态和 WebView 日志；guest 在传输前计算每个有界 payload 的哈希，coordinator 验证路径与身份后通过认证的 PowerShell Direct session 逐文件复制，并在 evidence writer 接受前于 host 独立复核字节数和 SHA-256。任何截断、缺失字节、哈希漂移、制品重名、采集失败、环境/诊断采集失败或关闭失败都会使整次 run 失败；如果较晚发生失败，先前已经采集的制品按 `failure` 而不是 `sample` 策略保留。第一次诊断运行 evidence key `0d6d2cb791f607294a9d66102e243e5ab3e61b72c39b3b3b467fab06ac261165` 暴露 2 MiB DOM 截断，因此不是资格证据；随后坚持完整 DOM 的控制台通道运行在 120 秒预算后按规则失败并保留 evidence key `c5cc3386f0fa512d8ae77dec8e3f0edf5dcab4e144e297e83b88286a2456bb55`。直接文件传输替代方案对同一真实 production 场景和候选 `50b3b36ffbdc95eebf1588ec80a7fe258ab7681ec094925ce6db49b400b3a308` 以 evidence key `66e2698709c04187c8f238efb1d07fccf435133f2f21ecf47c61a5a7410807df` 通过：manifest 中六个对象（报告加五个 payload）实算哈希全部一致，6,912,415 字节 DOM 以完整文档闭合标签结束，没有 payload 报告截断，VM 最终回到 `Off`。影响图选择了全部六个正式平台场景，并在最终 fail-closed 保留策略更新后重新完成资格运行：Playwright 场景以 `7cb8c2ac4c676d35ff13d1de9fac983c8062d0cb08fc24071a3fbe9bb386ea86` 通过并保存含 trace 的六个 payload；五个 production 场景分别以 `a36d4e3a1740091726b35671cab3e7f738f2ab2b99f9fd779bfecbe8de55f0f6`、`fda0802af92104c966e33b147404639a9ee3fbe7bc5c8ec7970e8b593e28616e`、`f22ee3b2dd1937028bd4acd6478bbe82215c650992bc7a96e5e23b73338818bf`、`6fe73889df63f26ffdf7fd23748ad95c0a33e1d1b52bb2f1c5c680e2adc6ccce` 和 `fe24b02fb018f88276b064606b112b293c9602b49204dbfe595bb551643b4d8d` 通过完整五 payload bundle。所有 manifest 对象实算哈希一致、没有 payload 截断、没有场景诊断，且每次 production run 都把 VM 恢复为 `Off`。production 性能 trace、视频、崩溃 dump 和逐动作截图仍是明确的制品工作；这些结果不把相应 bundle 冒充完成。
