...
生产场景 `core.text.multi-property-persistence.production` 从空白文档开始，使用受守卫的 Windows 输入激活公开 Text 工具，在画布两个独立位置点击，分别输入 `ChemSema H2O` 与 `Second text`，提交两个对象，再用 `Ctrl+A` 选择两者。随后只通过公开嵌套右键菜单批量应用 Bold、Times New Roman、Italic、Underline、Outline、Shadow、先 Superscript 后 Subscript、Right、24 pt 和 Green，用 `Ctrl+Z`/`Ctrl+Y` 证明颜色事务，经 Windows 原生对话框保存，通过有界且带 SHA 校验的通道回传 CCJS，并独立检查两个对象的 id、精确文本、对象级属性、样式和每套实际持久化的 run 集合。
...
随后主动收紧了已经绿色的持久化检查：文本颜色修改此前会更新当前 `runs` 与 style，却留下过期的 `sourceRuns`，未来重新编辑或重建时存在颜色回退风险。颜色与文本样式修改现在会更新所有存在的 `runs`、`sourceRuns` 与 `displayRuns` 集合；保存文档 oracle 会拒绝过期的 run 颜色、字体、字号、权重、斜体、下划线、描边、阴影或 script。后续保留证据 `0ae178e9bcdbcffbbbafa270d0122d3596f9fbac6bc0b41d880067b99c024a7e` 完成 42 个动作后暴露第二个 oracle 假设：Outline 正确使用 `fill="none"` 并把颜色承载在 `tspan` 的 stroke 上，因此历史完成条件现在精确检查两个渲染 stroke，而文件 oracle 仍独立于 SVG 表现。

候选 SHA-256 `b0a8361a7c81129e1a813377c59b6043c31663a962ef1b6d6eda9177568b579f` 以 evidence key `450cf701ea27695d9b992d196916ef2aa3b9e597d2536cee68c6d2622d155002` 通过全部 49 个真实输入动作和 3 个最终 oracle，诊断为 0。直接检查保存 CCJS，两个对象在实际持久化的 `runs` 与 `sourceRuns` 中均精确为 Times New Roman、24 pt、右对齐、权重 700、斜体、下划线、描边、阴影、下标和 `#008000`；这些对象没有 `displayRuns`，因此不虚报第三套集合。四场景影响闭包同时通过箭头属性 `5c7e7302e180ae62ded81ee391141cc24788c2709c8c8fa826e615deb318f9f0`、保存/重开 `67859c8954e184b41f5fd17fccf7de6d60a52dcb2ed4a7e63699936a48259e4e`、行距 `e7ce93f24b6cd07645852e615cdd89e47073e4a81cbcac99f950bc9d7bce2041` 和本场景。独立审计确认 126 个完成动作、12 个通过 oracle、0 诊断，以及 32 个制品共 41,693,846 字节的大小与 SHA-256 全部一致；全部报告绑定同一候选，VM 最终为 `Off`。这关闭的是上述已选样式状态，不代表其他所有取值、Formula、选区/既有文本编辑、锁定/组合/混合状态、格式边界、复杂/大文档、环境、耐久或连续 1,000 次资格目标已经完成。
...
登记闭包现已扩展为 19 个场景。`core.text.existing-edit-history.production` 创建 `Original text`，重新打开既有对象，选择完整编辑范围并替换为 `Edited text`，证明精确的撤销与重做转换，再输入第二个替代文本并用 Escape 取消，最后经 Windows 原生对话框保存。其完成合同有意保持严格：有界 selector 必须且只能匹配一个元素，其未经归一化的 `textContent` 必须逐字与声明字符串相等。保存文档 oracle 独立要求 `Edited text`，并要求每套实际存在的持久化 run 集合中粗体权重均为 700。第一次生产运行以 evidence `c04f0ee9565e369b2a4fc7504df309bf37a30500a063b21a40eff3b36e5d9b82` 在首个编辑动作前失败，因为新 guest bridge 错把只适用于计数请求的表达式路由进文本请求；该协议缺陷已修复并加入回归测试。随后 evidence `a62adad47589def9fa3f42b3df6ab6f4e8f4c54dd29b81f7e14e5ef26e4c7591` 完成全部 24 个真实输入动作、12 次精确文本转换和 3 个最终 oracle，诊断为 0。这关闭的是整对象既有文本替换、历史、Escape 取消和持久化；局部选区样式、光标边缘情形、IME/组合输入、Formula、锁定/组合/混合文本仍开放。

首次 19 场景当前候选资格尝试被明确记录为失败，即使已有 18 个唯一场景具备通过证据。锁定祖先组合场景在完成 16 个动作后失败，evidence 为 `598aecc048754254ad29189afa1176c35f97b0b61f35f61133d6319b3c337a97`：最终截图已显示 Canvas Menu 打开且 `Unlock` 可见，但持久 CDP 通道在 8 秒内没有返回回执；其性能轨迹包含 6,135.102 ms 的主线程任务和 5,779.684 ms 的排队输入延迟。保存/打开场景另以 evidence `1d2dceacbe5ba80c6cbe5e524fdef0d5b841f245e7fad5ee0c31fd7c90c36800` 在 30,004 ms 失败；文档实际上已经重开且显示一根键，但完整原生对话框事务超过未改变的 30 秒动作预算。两次失败均未删除、未改写成通过，也没有通过增加超时隐藏。

动作收据现在以有界结构记录目标解析、输入、原生对话框消失、交互代理重新认证、产品完成判定和最终状态采集的分阶段时延。每个生产 evidence bundle 还会从完整 gzip trace 派生有界的 `chemsema.gui.performance-summary.v1` 制品，保留事件总数、全部大于等于 50 ms 的长任务统计、最长 25 个事件，以及渲染/输入热点汇总。平台发生这一实质变化后，同一可执行候选的一次定向保存/打开资格以 evidence `a479df1bb10e6664947f6d17a5cc8e4d30651af7a7e81d0c3de4910e890eead2` 通过 17/17 动作和 3/3 oracle。`confirm-open` 用时 16,886 ms：目标解析 4,937 ms、输入 2,299 ms、原生对话框消失 4,159 ms、重新认证 3,299 ms、完成观察 2,192 ms。其摘要覆盖 266,366 个 trace 事件，观察到的最长长任务为 175.519 ms。后续通过证明该次运行的功能正确性，但不会抹掉前一次长尾可靠性失败。
