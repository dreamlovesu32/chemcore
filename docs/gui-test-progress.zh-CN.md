...
当前源码闭包已登记场景资格：**31/32 passed，0 failed，1 pending，0 diagnostics**。31 场景 registry 已取得不可变的 31/31 qualification；影响图下一项 Chain 端点附着与继续绘制 production 单元已登记并交给独立后台批次。完整 GUI 功能矩阵仍未完成，本文总体状态不变。
...
| 🟡 | 脱离 Codex 的连续后台队列 | 单批执行器已有单实例租约、15 秒心跳、PID 清单、提交/候选/profile/queue 哈希绑定、逐场景 checkpoint、资源暂停、停止请求和 evidence manifest 哈希；当前候选 26 场景物理批次、浏览器基线、十键 Bond、六种平面环、Chair/Benzene 及可变长度 Chain 均已通过，终态事件已连续驱动失败修复和完成续接；Chain 端点附着续画单元待后台续接，且仍需 supervisor/子进程重启故障注入和长期终态唤醒验收 |
...
| 状态 | 功能族 | 已有真实覆盖 | 明确剩余 |
|---|---|---|---|
| 🟡 | 分子、原子、键 | 单键绘制、历史、多键/混合选择与剪贴板；十种非单键工具与四键 Chain 已取得真实 OS 输入与精确持久化证据 | 元素/标签/电荷/氢、环的附着与继续绘制、Chain 端点附着续画、模板、端点反转、双键位置循环、立体化学、反应语义、全部属性与格式 |
...
| 🟡 | Rings、Chain、Template Library | 六种平面环、双 Chair、Benzene 与四键可变长度 Chain 已取得真实输入、精确拓扑与持久化证据；Chain 端点附着续画已登记 | 端点附着续画 production 证据、环连接、其他长度/相位、每种公开库模板、属性、历史、其他格式、搜索/库切换 |
...
所有 32 个场景均已实现并进入 registry。当前不可变候选已取得前 31 个场景的完整 qualification；随后按影响与功能缺口新增 Chain 端点附着与继续绘制 production 场景，因此当前 32 场景 registry 为 31/32，新增场景待独立后台证据。这一 registry 闭包即使最终全绿，也不表示对应功能族或本文列出的完整 GUI 矩阵已经覆盖。
...
| 🟡 | `core.chain.endpoint-attachment-continuation.production` | 从已存在端点继续真实 OS Chain 拖拽，要求八键仍为单一九节点分子并精确持久化；后台批次待完成 |
...
- 扩展前 registry 的 31 场景已有单候选完整资格；当前 32 场景 registry 的新增 Chain 端点附着续画 production 场景待跑。这不关闭尚未登记的功能、属性、格式、规模、环境和稳定性缺口。
...
2. 🟡 **化学绘制主干**：十种非单键工具、六种平面环、双 Chair、Benzene 与可变长度 Chain 精确语义批次已完成；当前先跑 Chain 端点附着续画，再推进原子/标签/电荷、Template Library、反应连接与属性。
...
- 每新增、删除或重命名一个 registry 场景，必须同步修改“登记场景”数字和第 4 节表格；自动测试会逐个检查 32 个场景 ID，漏项直接失败。
