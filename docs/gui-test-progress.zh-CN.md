...
当前源码闭包已登记场景资格：**25/33 reusable passed，0 product failed，8 pending，0 qualification diagnostics**。32 场景 registry 的不可变 32/32 qualification 保持有效历史；首次六元环 Bond 融合运行在点击已选 Ring 6 时暴露复合工具主按钮与二级选项同名的测试定位缺陷，失败证据永久保留。通用定位规则和 8 个受影响场景已改为限定 `Secondary toolbar`，正在重跑该精确闭包。完整 GUI 功能矩阵仍未完成，本文总体状态不变。
...
| 🟡 | 脱离 Codex 的连续后台队列 | 单批执行器已有单实例租约、15 秒心跳、PID 清单、提交/候选/profile/queue 哈希绑定、逐场景 checkpoint、资源暂停、停止请求和 evidence manifest 哈希；当前候选 32 场景历史 qualification 已通过。六元环 Bond 融合首次运行在同名主/二级控件上 fail closed，失败证据保留；通用二级工具栏 scope 修复后的 8 场景受影响闭包正在后台执行，且仍需 supervisor/子进程重启故障注入和长期终态唤醒验收 |
...
| 状态 | 功能族 | 已有真实覆盖 | 明确剩余 |
|---|---|---|---|
| 🟡 | 分子、原子、键 | 单键绘制、历史、多键/混合选择与剪贴板；十种非单键工具、四键 Chain 及 Chain 端点续画已取得真实 OS 输入与精确持久化证据 | 元素/标签/电荷/氢、环与既有 Bond 融合、环端点附着与继续绘制、模板、端点反转、双键位置循环、立体化学、反应语义、全部属性与格式 |
...
| 🟡 | Rings、Chain、Template Library | 六种平面环、双 Chair、Benzene、四键可变长度 Chain 与 Chain 端点附着续画已取得真实输入、精确拓扑与持久化证据；六元环 Bond 融合已登记 | 环融合 production 证据、环端点附着与继续绘制、其他长度/相位、每种公开库模板、属性、历史、其他格式、搜索/库切换 |
...
所有 33 个场景均已实现并进入 registry。当前不可变候选已取得前 32 个场景的完整 qualification；随后按影响与功能缺口新增六元环与既有 Bond 融合 production 场景，因此当前 33 场景 registry 为 32/33，新增场景待独立后台证据。这一 registry 闭包即使最终全绿，也不表示对应功能族或本文列出的完整 GUI 矩阵已经覆盖。
...
| 🟡 | `core.ring.bond-fusion-persistence.production` | 先真实绘制单键，再于键中点插入六元环，要求共享键去重并持久化为单一六节点环；首次运行保留测试定位失败，修复后二级工具栏精确闭包待完成 |
...
- 扩展前 registry 的 32 场景已有单候选完整资格；当前 33 场景 registry 保留六元环 Bond 融合的首次 test-locator 失败，二级工具栏通用 scope 修复使 8 个场景进入受影响闭包。这不关闭尚未登记的功能、属性、格式、规模、环境和稳定性缺口。
...
2. 🟡 **化学绘制主干**：十种非单键工具、六种平面环、双 Chair、Benzene、可变长度 Chain 与端点续画精确语义批次已完成；当前先跑六元环 Bond 融合，再推进环端点附着、原子/标签/电荷、Template Library、反应连接与属性。
...
- 每新增、删除或重命名一个 registry 场景，必须同步修改“登记场景”数字和第 4 节表格；自动测试会逐个检查 33 个场景 ID，漏项直接失败。
