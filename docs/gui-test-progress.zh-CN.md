...
当前源码闭包已登记场景资格：**28/29 passed，0 failed，1 pending，0 diagnostics**。扩展前的 28 场景 registry 已取得不可变的 28/28 qualification；影响图下一项六种平面环工具 production 持久化矩阵正在等待独立后台批次。完整 GUI 功能矩阵仍未完成，本文总体状态不变
...
| 🟡 | 脱离 Codex 的连续后台队列 | 单批执行器已有单实例租约、15 秒心跳、PID 清单、提交/候选/profile/queue 哈希绑定、逐场景 checkpoint、资源暂停、停止请求和 evidence manifest 哈希；当前候选 26 场景物理批次、浏览器基线和十键 Bond 矩阵均已通过，终态事件已连续驱动失败修复和完成续接；新增六种平面环 production 矩阵待后台续接，且仍需 supervisor/子进程重启故障注入和长期终态唤醒验收 |
...
| 状态 | 功能族 | 已有真实覆盖 | 明确剩余 |
|---|---|---|---|
| 🟡 | 分子、原子、键 | 单键绘制、历史、多键/混合选择与剪贴板；十种非单键工具已取得真实 OS 输入与精确持久化证据 | 元素/标签/电荷/氢、环的附着与继续绘制、链、模板、附着与端点反转、双键位置循环、立体化学、反应语义、全部属性与格式 |
...
| 🟡 | Rings、Chain、Template Library | 六种平面环 production 持久化矩阵已登记待跑 | Chair、Benzene 精确键级、连接、继续绘制、Chain、每种公开库模板、属性、历史、持久化、搜索/库切换 |
...
所有 29 个场景均已实现并进入 registry。当前不可变候选已取得前 28 个场景的完整 qualification；随后按影响与功能缺口新增六种平面环工具 production 持久化场景，因此当前 29 场景 registry 为 28/29，新增场景待独立后台证据。这一 registry 闭包即使最终全绿，也不表示对应功能族或本文列出的完整 GUI 矩阵已经覆盖。
...
| 🟡 | `core.ring.six-planar-persistence.production` | 六种公开平面环的真实 OS 点击、累计拓扑、原生保存与精确节点/键/分子计数；后台批次待完成 |
...
- 扩展前 registry 的 28 场景已有单候选完整资格；当前 29 场景 registry 的新增六种平面环 production 场景待跑。这不关闭尚未登记的功能、属性、格式、规模、环境和稳定性缺口。
...
2. 🟡 **化学绘制主干**：十种非单键工具精确语义批次已完成；当前先跑六种平面环，再推进原子/标签/电荷、Chair/Benzene、Chain、Template Library、反应连接与属性。
...
- 每新增、删除或重命名一个 registry 场景，必须同步修改“登记场景”数字和第 4 节表格；自动测试会逐个检查 29 个场景 ID，漏项直接失败。
