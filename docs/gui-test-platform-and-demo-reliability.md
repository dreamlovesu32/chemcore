...
The Hyper-V module is present and `vmms`/`vmcompute` are running on a host with 24 logical processors and about 63.4 GiB RAM. `jiajun\dream` is now enabled in `Hyper-V Administrators`. The existing Windows 11 test VM (document alias `windows-gui-worker-current`) is Generation 2 with 8 vCPU and 4–20 GiB dynamic memory; its configuration, automatic checkpoint, and VHDX/AVHDX chain were readable. On 2026-08-08 it was started from Off to Running, reported healthy heartbeat/time/KVP/shutdown integration, and was then normally stopped; Hyper-V completed the automatic-checkpoint merge and returned it to Off/healthy state. Office activation is user-confirmed but was not independently checked in the guest. The Default Switch produced only a `169.254.*` address, so guest DHCP/networking remains unresolved or must be deliberately isolated. PowerShell Direct commands, file round trips, and UI-input isolation still require dedicated guest credentials before the full runner can be called operational.
