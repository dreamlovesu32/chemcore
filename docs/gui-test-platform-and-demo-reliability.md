...
### Real text creation, batch properties, and persistence

The production scenario `core.text.multi-property-persistence.production` starts from a blank document and uses guarded Windows input to activate the public Text tool, click two independent canvas positions, type `ChemSema H2O` and `Second text`, commit both objects, and select both with `Ctrl+A`. It then uses only public nested context menus to apply Bold, Times New Roman, Italic, Underline, Outline, Shadow, Superscript followed by Subscript, Right alignment, 24 pt, and Green. It proves the color transaction through `Ctrl+Z`/`Ctrl+Y`, saves through the native Windows dialog, transfers the CCJS through the bounded SHA-verified channel, and independently checks both object ids, exact text, object-level properties, styles, and every persisted run collection.

The retained failures exposed three product defects and one test-oracle defect. Evidence `9cf97c506326398cf34ac922a7865d40b14d6347ae17f68740d03cc9bd87d89f` and `eb3764d3ff2f042e038de14996a007c5cd800035a35c0e05cc798be20c35faea` showed that a zero-sized pre-layout container collapsed the initial editor viewBox to `2.2 × 2.2`; invalid dimensions now use the full default workspace and point-only content bounds cannot trigger Fit View. Evidence `33e5139e4ba8d4342c52204e9a205604b63b0cc7e070491047733f5c8d659a84` showed that homogeneous multi-text selection lacked public property menus and that Bold wrote `fontWeight` as floating JSON, which made the typed render runs unreadable; multi-text menus now expose Font, Style, Size, Alignment, and Line Spacing, while text weight is written as a strict integer and verified through document, render-list, and checked-menu projections. Evidence `f46c277c67d47b2464f0e5dbc6d214c65d9478d2028fd544f790c7e9de8d3f30` found a Color submenu extending into the Windows taskbar work area; main menus and nested submenus now use `screen.avail*`, shift above the bottom work-area boundary, and flip left at the right boundary without weakening the foreground-process guard. Evidence `a397f9b17ac1a1f2e6da42ef40d67d8e80821238dceee43f79d68c6777543ed0` proved Undo was correct but the completion selector still assumed unformatted single-node text; formatted run color is now observed on the actual `tspan`.

The initially green persistence check was deliberately strengthened: applying a text color had updated the live `runs` and style but left `sourceRuns` stale, creating a future edit/reconstruction rollback risk. Color and text-style mutations now update every existing `runs`, `sourceRuns`, and `displayRuns` collection, and the saved-document oracle rejects stale run colors, font families, font sizes, weights, italics, underlines, outlines, shadows, or scripts. The later retained evidence `0ae178e9bcdbcffbbbafa270d0122d3596f9fbac6bc0b41d880067b99c024a7e` reached 42 completed actions and exposed a second oracle assumption: Outline correctly renders `fill="none"` and carries color in the `tspan` stroke, so history completion now checks the exact two rendered strokes while the file oracle remains independent of SVG representation.

On executable SHA-256 `b0a8361a7c81129e1a813377c59b6043c31663a962ef1b6d6eda9177568b579f`, evidence key `450cf701ea27695d9b992d196916ef2aa3b9e597d2536cee68c6d2622d155002` passed all 49 real-input actions and all 3 final oracles with no diagnostics. Direct inspection found both objects exactly Times New Roman, 24 pt, right-aligned, weight 700, italic, underlined, outlined, shadowed, subscript, and `#008000` in both persisted `runs` and `sourceRuns`; `displayRuns` was absent from these objects rather than being falsely claimed. The four-scenario impact closure also passed arrow properties (`5c7e7302e180ae62ded81ee391141cc24788c2709c8c8fa826e615deb318f9f0`), save/open (`67859c8954e184b41f5fd17fccf7de6d60a52dcb2ed4a7e63699936a48259e4e`), line spacing (`e7ce93f24b6cd07645852e615cdd89e47073e4a81cbcac99f950bc9d7bce2041`), and this text matrix. An independent audit found 126 completed actions, 12 passing oracles, zero diagnostics, and 32 artifacts totaling 41,693,846 bytes with exact sizes and SHA-256; all reports bound the same candidate and the VM finished `Off`. This closes these selected style states, not every remaining value, Formula mode, selection-range or existing-text editing, locked/grouped/mixed states, format boundaries, complex/large documents, environments, endurance, or 1,000-repeat qualification.

The next exact candidate closure retained another infrastructure-boundary failure instead of retrying it. In `core.group.locked-ancestor-transform.production`, evidence `a22706bdc028ba652b637005951a0a8787fbaa6deef1d2b4d573d4bc5419559e` completed 17 actions and resolved the visible Canvas Menu `Unlock` target, but the old 12,000 ms outer wrapper expired at 12,015 ms before guarded input was submitted. This proved that applying the 30-second envelope only to geometry-delta actions was incomplete. The protocol now requires at least 30,000 ms for every ordinary end-to-end action transaction and at least 90,000 ms for native document saves, while each product completion predicate retains its original functional timeout. Scenario Schema, guest transaction Schema, the Hyper-V runtime guard, and every registered scenario enforce the same boundary. Increasing the transport envelope therefore cannot conceal a slow product transition: the unchanged inner completion still fails first and returns its precise diagnostic.

The current-candidate exact closure then passed on executable SHA-256 `f7891211ac0af791c27ce9705c52f35380f3eca8bcb67af8f1169323abf039ba`, bound to product source closure `a3f867d854040906879602bd011aa24cf5c8aa5bfc440cd8e7e005aca2910991`: browser single-bond `e876d155ba25e503752b6f66e0561a59a733451ed622e1832b88e5840030a26a`; production single-bond `7eb26b79e6ffdd2256b0c0d602464bac57bee6db713f4f9d6946d17f8ae84d2a`, history `2fe22b4aeea827b3f4c8b6b7a6ddca92ba93a306933c537b011f47da5b34516e`, multi-bond clipboard/delete `6e82f6b99d230cb64dda2bf5319ff53378d910fedb4cf5895b6249f46ed24c69`, mixed bond/arrow clipboard/delete `19dd654a089d4265180f6cf6c0e6e69379f136b23df59689cba979796f66d9ee`, region/additive selection `0cda2a6fe5bf7bb2cd62625645f32538d4ffd50f1228755ad0ffe2e6ddab44b4`, multi-arrow history `5b1ebec9610ed8322a568dd85d18bd8ddbc29cf510cda4a13cbe6dfc6acfef94`, arrow persistence matrix `5b31e95bdeffbef8697818ae0ce00886a7718e2da0977e6019c46719065c5234`, locked mixed-arrow properties `27b4c014adeacb35dab07e1ab7da5ca54afa43001c1ffbc01a2f025c96c0e22f`, locked partial deletion `0c7399b1e4299930549d96cbd8ae5ab649c35ac1300b9b63eefd77c6520d2a2d`, locked transform `dc25364146d6f87e1dd8fc0f6c0bc35e01e8f8678a780f720529b494bcd21384`, locked molecule/arrow transform `230fad270d27a9c1f2e0a664cba67d3aa5c35871b617d83bdfd1c09134d0ce23`, cross-document clipboard `9da6512ff1f9e13e2d2bf60820c32bb0353933f2faeaec3730248c2751dbd06b`, nested mixed-group clipboard `b87ed94e9afa8ec9c04c6d862c5b446952365a9e677045470fb4c38162ecf92a`, locked ancestor-group transform `e5501c1579d488a4b45013c0a27bc6f37d6c052597863f51021eb6f3fb9149ac`, save/open roundtrip `fc30b7dfb76866d5023837fac046cd6c232a9937a62f619048615533a8f93d35`, and text property persistence `15799f2b1ac3207c35846afba396b4b1f7f04e30d1b62451be46f7b18f4c5970`. An independent audit found 17 unique scenario/driver pairs, 290 completed actions, 57 passing final oracles, zero failed statuses/actions/oracles/diagnostics, and one candidate across all 16 production reports. All 109 manifest artifacts, totaling 149,072,097 bytes, were reread and matched their declared sizes and SHA-256. The isolated VM finished `Off`, with eight configured processors, zero assigned memory, and no `vmwp` process. This qualifies the current 17-scenario closure; it does not close the still-explicit remaining object/property, format, complex/large/xlarge, environment-matrix, endurance, or 1,000-repeat goals.

### Multiline line-spacing validation and precise incremental selection

Production scenario `core.text.line-spacing-validation.production` now creates a two-line text object from a blank document through guarded Windows input, including a real Enter key event. It opens the public Line Spacing dialog by accessible name, proves that `-1` is rejected without committing a history entry, applies fixed line height `20`, verifies undo restores `12`, verifies redo restores `20`, cancels both inspection dialogs without mutation, then saves through the native Windows dialog. The saved-document oracle requires the exact text `First line\nSecond line`, `lineHeight: 20`, and `lineHeightMode: "fixed"`; it does not infer success from dialog state or pixels. Numeric dialogs now expose a named modal-dialog role and a named input, and their generated markup has direct escaping and accessibility tests. Evidence key `cddd49f1789bcacd43a9f5e5e933be5626936465009823f0ae128f5b53b8189d` completed all 27 actions and all 3 oracles with no diagnostics.

The impact graph now distinguishes runtime code from test-only entrypoints, metadata from executable runner code, the document-file oracle from the production driver, and `viewer/numeric_dialog_host.js` from the rest of `viewer/**`. Source patterns support explicit exclusions. A numeric-dialog-host-only change therefore selects exactly the line-spacing production scenario; a document-file-oracle change selects exactly the four scenarios that consume that oracle; a `scripts/test.mjs`-only change selects no GUI production scenario. Changes to the selector itself still invalidate the full registered closure, as they must. This precision is executable regression-tested behavior, not a documentation-only promise.

Because this unit changed the selector, the one-time exact closure executed all 18 registered scenarios against executable SHA-256 `b0a8361a7c81129e1a813377c59b6043c31663a962ef1b6d6eda9177568b579f`, bound to source closure `cec820508e990f37ee345da2255e035b253e887f129c263e1686e85c676963b8`. All 18 passed on their first execution: 17 production black-box scenarios and one Playwright browser baseline. An independent audit reread 317 completed action receipts, 60 passing oracles, and 117 manifest artifacts totaling 159,179,466 bytes; it found zero diagnostics, zero failed statuses/actions/oracles, zero candidate mismatches, and zero size or SHA-256 mismatches. The isolated VM finished `Off` with zero assigned memory. This closes the registered 18-scenario closure and the exact multiline fixed-line-spacing cell only; the remaining text property/value matrix, other object families, format boundaries, complex/large/xlarge documents, environments, endurance, and 1,000-repeat demonstration qualification remain open.

### Existing-text editing and latency-attributed qualification

The registered closure now contains 19 scenarios. `core.text.existing-edit-history.production` creates `Original text`, reopens the existing object, selects the complete edit range, replaces it with `Edited text`, proves exact undo and redo transitions, types a second replacement and cancels it with Escape, then saves through the native Windows dialog. Its completion contract is deliberately exact: a bounded selector must match exactly one element and its unnormalised `textContent` must equal the declared string byte-for-byte. The saved-document oracle independently requires `Edited text` and bold weight 700 in every persisted run collection that exists. The first production attempt, evidence `c04f0ee9565e369b2a4fc7504df309bf37a30500a063b21a40eff3b36e5d9b82`, failed before the first editor action because the new guest bridge routed a count-only expression into the text request. That protocol defect was fixed and regression-tested. Evidence `a62adad47589def9fa3f42b3df6ab6f4e8f4c54dd29b81f7e14e5ef26e4c7591` then completed all 24 real-input actions, twelve exact text transitions, and all three final oracles with no diagnostics. This closes whole-object existing-text replacement, history, Escape cancellation, and persistence; partial-range styling, caret-edge cases, IME/composition, Formula mode, and locked/grouped/mixed text remain open.

The first 19-scenario current-candidate qualification attempt is intentionally recorded as failed, even though 18 unique scenarios have passing evidence. The locked ancestor-group scenario failed after 16 completed actions with evidence `598aecc048754254ad29189afa1176c35f97b0b61f35f61133d6319b3c337a97`: the final screenshot showed the Canvas Menu open with `Unlock` visible, while the persistent CDP channel returned no receipt within 8 seconds. Its trace contained a 6,135.102 ms main-thread task and 5,779.684 ms queued-input delay. The save/open scenario separately failed at 30,004 ms with evidence `1d2dceacbe5ba80c6cbe5e524fdef0d5b841f245e7fad5ee0c31fd7c90c36800`; the document had in fact reopened with one bond, but the complete native-dialog transaction crossed its unchanged 30-second action budget. Neither failure was removed, reclassified as a pass, or hidden by increasing a timeout.

Action receipts now expose bounded phase timings for target resolution, input, native-dialog dismissal, interactive-agent re-attestation, product completion, and final-state capture. Every production evidence bundle also derives a bounded `chemsema.gui.performance-summary.v1` artifact from the full gzip trace, retaining the event count, all >=50 ms long-task statistics, the 25 longest events, and aggregate rendering/input hotspots. After this platform change, one targeted save/open qualification on the same executable passed 17/17 actions and 3/3 oracles with evidence `a479df1bb10e6664947f6d17a5cc8e4d30651af7a7e81d0c3de4910e890eead2`. `confirm-open` took 16,886 ms: 4,937 ms target resolution, 2,299 ms input, 4,159 ms native dismissal, 3,299 ms re-attestation, and 2,192 ms completion observation. Its summary covered 266,366 trace events, with an observed maximum long task of 175.519 ms. This later pass proves functional correctness for that run but does not erase the earlier long-tail reliability failure.

An independent audit of the 18 reports in the attempted closure plus the existing-text pass and the phase-attributed save/open pass reread 20 reports, 372 action receipts, 59 oracle results, and 134 manifest objects totaling 180,619,707 bytes. All 134 objects matched their declared byte size and SHA-256. The set contains 18 passed reports and the two retained failures above; across unique registered scenarios, only the locked ancestor-group cell lacks a clean result in this attempted qualification. The VM finished `Off`, with eight configured processors and zero assigned memory. Consequently this is expanded coverage, not a passing 19-scenario qualification and not completion of the remaining object/property/value, format, complex/large/xlarge, environment, fault/model/mutation, endurance, or 1,000-repeat goals.

The trace protocol now writes four bounded user-timing markers inside each ordinary guest action transaction: `start`, `input-before`, `input-after`, and `complete`. They share the same persistent observer channel as the before/completion/after observations and never replace guarded OS input. A new diagnostic execution of the locked ancestor-group scenario on the same candidate passed all 22 actions and four oracles with evidence `b07c0ced9a0bf1576f0130e7fbedac9c10bbb6aec2d152fce8c2c97fecebd6d4`. All expected 88 markers were present without truncation. Across 164,998 trace events it contained no task at or above 50 ms; Layout, Paint, and Commit peaked at 1.139 ms, 0.412 ms, and 5.286 ms, compared with 2,094.164 ms, 1,298.080 ms, and 614.156 ms in the retained failure. Host Hyper-V and System logs contained no scheduling, CPU, or memory fault at the failure action. A recurring VHDX security-information warning appears at startup of both passing and failing runs and therefore cannot explain this event. The evidence supports a whole-renderer or VM scheduling pause, but does not yet distinguish those causes. The later pass supplies a clean functional result; it does not turn the failed qualification attempt green.

That policy is now executable through `gui-platform qualify`. The command validates every supplied run report, requires every registered scenario, requires exactly one production candidate hash, rejects unexpected or duplicate identities, locates each evidence manifest by evidence key and run id, confines every object URI to the evidence root, and rereads every byte before comparing declared size and SHA-256. A scenario remains failed when any included run failed, even when a later run passed. The current 21-report qualification result is therefore `failed`: all 19 scenarios are present, 17 scenario cells are clean, save/open and locked ancestor-group each contain one failed plus one passed run, and the two failed runs remain blocking. The gate independently verified 141 manifest objects totaling 189,509,350 bytes with no evidence diagnostic or candidate mismatch. Qualification output itself is a strict versioned `chemsema.gui.qualification.v1` document; a non-passing result exits nonzero.

### Four-kind shape creation, batch properties, history, and persistence

The registered closure now contains 20 scenarios. `core.shape.multi-kind-style-history.production` uses guarded real mouse input to draw Circle/Solid, Ellipse/Dashed, Rounded rectangle/Shaded, and Rectangle/Filled objects. It selects all four, applies Shadowed through the public canvas menu, proves the uniform checked state, undoes the batch transaction, explicitly reselects after the document-snapshot history contract clears selection, proves the four original mixed styles are restored, redoes Shadowed, and saves through the native Windows dialog. The independent CCJS oracles require format 0.2, the expected empty editor molecule plus four graphics, exact persisted kinds `circle`, `ellipse`, `roundRect`, and `rect`, and for every shape `fill: null`, black stroke, width 1, empty dash array, `shaded: false`, `shadow: true`, and `shadowSize: 4`. Candidate `59d280714265b53527ed5984131423200134ceeede3ee84eac00123f06e7620b` passed 30/30 real-input actions and 4/4 final oracles with no diagnostics; evidence key `c10ae65a412ed6a91e273852cfa448eb5086d88152fedd3a30ca7708284c447b` retains eight artifacts and the VM returned to `Off`.

The failure trail is retained. Evidence `3e08a7179b946a2f7d82f531f8a6ab6d62d4236fa948ace1ce3159b426ac7613` and `8c92962773930e111df709a2eef837a25c6457977c0795748d97be4d467b7ab1` corrected unsupported DOM assumptions. Evidence `8dce1a44c68bdaa94f56ffe10e33ffc92bf18f9f2a8fc351e33fb87f8e3a4df0` exposed the real product defect: a homogeneous multi-shape selection had no public Shape Style menu even though the engine supported batch application. The Rust menu branch and regression test now cover mixed initial styles, uniform application, selection-clearing undo, reselection, and exact mixed-style restoration. Evidence `2f6932acfd90b696ffc91d74e888371bc55180bbc43e6be0ea18461374259e51`, `2d78c96a6b02a623dae0e07828e9f1e9d15e09c4abf7cdc2f0128eb0fe47d28a`, and `b5f1045845f0898f1b429bf96430c47d8b143f53ba3da4d58b7f888d5267faae` then corrected explicit selection, empty-editor-root counting, and UI-versus-CCJS kind naming without weakening product assertions. The full repository gate subsequently passed, including 146 JavaScript tests, all Rust and documentation tests, WASM rebuild, and generated-artifact synchronization.

This is one qualified shape cell, not a green 20-scenario candidate qualification. The fail-closed current-candidate qualification document reports 20 expected, one passed, 19 missing, zero failed runs, and nine rehashed evidence objects totaling 10,539,578 bytes. The engine change invalidates the affected prior-candidate evidence closure, and the earlier 19-scenario qualification remains failed under the retained-failure policy. Colors, Faded, locking/grouping, transforms, clipboard, other formats, complex/large/xlarge documents, environment/fault/model/mutation coverage, endurance, and 1,000-repeat demonstration qualification remain open.

### Eight-kind charge/electron symbols, batch color, history, and persistence

The registered closure now contains 21 scenarios. `core.symbol.eight-kind-color-history.production` activates the public Charge and electron symbols tool and creates Circle plus, Plus, Radical cation, Lone pair, Circle minus, Minus, Radical anion, and Electron as eight spatially separate objects with guarded real pointer gestures. It selects all eight, applies Green through the public Color menu, proves the uniform checked value, undoes to uniform black, reselects after snapshot history clears selection, redoes to green, and saves through the native Windows dialog. The file oracle requires format 0.2, one empty editor molecule plus eight symbol objects, the exact eight persisted kind names, `symbolStyle: default`, and matching `#008000` values on both each payload and its referenced style.

Candidate `59d280714265b53527ed5984131423200134ceeede3ee84eac00123f06e7620b` passed 36/36 actions and 4/4 final oracles in 341,499 ms with no diagnostic. Run `e17d9f30-b612-495a-901c-d1a15d3f9f9f`, evidence key `93d8ec852d1dc25bc45f05e5c23de868f5034781e89e39264e44eb78072c1f23`, retains eight driver artifacts; the qualification reread nine manifest objects totaling 11,404,910 bytes, and the VM returned to `Off`. The earlier failed run remains retained under evidence key `fcf79b3feec54c7f5fb1534621551f785adf2bd93b05fa1ea7784176efff4412`: the product created the first symbol, while the test incorrectly assumed a `data-object-type` attribute on its renderer root. The corrected oracle counts distinct `data-object-id` values across the stable public `document-graphic` primitives, so multiple SVG paths cannot inflate the object count.

This remains one qualified current-closure cell, not overall qualification. `qualification-current-21-incomplete.json` is deliberately `failed`: 21 expected scenarios, one passed, 20 missing, zero failed runs, and no evidence diagnostic. Standalone symbols are covered here; atom/label attachment, orbit placement, resulting chemistry and Link relations, mixed/grouped/locked symbols, transforms, clipboard, CDXML/CDX round trips, and large-document symbol behavior remain explicit work.

### Paired brackets: public properties, visible-side updates, hierarchy, and transport isolation

The registered closure now contains 22 scenarios. `core.bracket.three-kind-properties-history.production` draws Parentheses, Square brackets, and Braces as three public paired objects. Each editor pair is represented by one `bracket-group` root and two visible `bracket` sides. Testing exposed a product defect: selection saw only the group root, so Bracket Type was absent and Color could be written to the invisible group instead of its rendered children. The engine now recognizes selected bracket groups as bracket property targets, derives uniform type/color from their visible descendants, and applies color to both side payloads. A Rust regression covers mixed initial kinds, public menu exposure, uniform type/color application, visible payload changes, and two-step undo.

The final isolated run `48d30648-dc4e-4f7b-a0f3-cf4fb0acb008` on candidate `cb3d367ba9bea4e278b33a25aebb7c60add65787ed343ea31a94c07508e2e42e` passed 29/29 guarded OS-input actions and 4/4 final oracles in 283,368 ms with no diagnostic. Evidence key `94318165141069e8e8f0f3e54b27e9fda9cef71d769bec23722b739a29763257` retains eight driver artifacts. The independent CCJS oracle requires format 0.2, one empty editor molecule plus three roots and six sides, exact `curly`/left-or-right/`#008000` side payloads, and exact file-header `hierarchy.children` arrays mapping every root to its own ordered left/right IDs. Qualification reread nine objects totaling 10,690,033 bytes, and the VM returned to `Off`.

The failure trail remains. Evidence `e0265830730310110bbfd866df8583b855e6a2b39d05b18d53a7e5d9639dfe75` recorded a persistent CDP-channel receipt timeout after the third draw. The platform previously used the product's 8-second completion window as the transport receipt limit; it now reserves a separate bounded 15-second target-resolution/transport envelope while retaining the original 8-second product observation and 30-second action budget. Evidence `18f0b35756855396618d46dde03915bb8f5e0bcefb66235e6ff7070d47977013` then passed the full workflow, but inspection showed its first oracle version checked side records without proving 0.2 hierarchy membership. That evidence was not accepted as final; the strengthened hierarchy oracle was rerun to produce the final evidence above.

`qualification-current-22-incomplete.json` remains deliberately `failed`: 22 expected, one passed, 21 missing, zero failed runs, nine verified evidence objects, and no evidence diagnostic. Bracket labels, repeat-count Link relations, molecule containment, handle resizing, lock/group interactions, clipboard, CDXML/CDX round trips, and large documents remain open.
