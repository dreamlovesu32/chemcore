...
The production scenario `core.text.multi-property-persistence.production` starts from a blank document and uses guarded Windows input to activate the public Text tool, click two independent canvas positions, type `ChemSema H2O` and `Second text`, commit both objects, and select both with `Ctrl+A`. It then uses only public nested context menus to apply Bold, Center, 18 pt, and Blue, proves the color transaction through `Ctrl+Z`/`Ctrl+Y`, saves through the native Windows dialog, transfers the CCJS through the bounded SHA-verified channel, and independently checks both object ids, exact text, Arial family, 18 pt size, centered alignment, integer weight 700, and blue style/run colors.
...
The initially green persistence check was then deliberately strengthened: applying a text color had updated the live `runs` and style but left `sourceRuns` stale, creating a future edit/reconstruction rollback risk. Color mutation now updates every existing `runs`, `sourceRuns`, and `displayRuns` collection, and the saved-document oracle rejects any inconsistent collection. Final source-bound candidate SHA-256 `4f2423b8800928a099134e326a4340bf6e2f6bb67e043afb5c4422de717077cd` passed all 28 real-input actions and all three final oracles with evidence key `d2bc0d549731e4fb782a87354d229c89869c682dca18bf3ceaeff4129b3c7134`. All eight manifest objects matched their declared sizes and SHA-256 values. Direct inspection of the saved CCJS found both text objects at weight 700 and `#0000ff` in both `runs` and `sourceRuns`; diagnostics were empty and the VM finished `Off` with zero assigned memory. The registry now contains 29 entries and 17 scenarios with no unexplained gaps or warnings. This closes this exact two-text property/persistence cell, not remaining text styles, scripts, fonts, alignments, values, editing modes, locked/grouped/mixed states, format boundaries, complex/large documents, environment matrices, endurance, or 1,000-repeat qualification.
