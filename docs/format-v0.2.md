...
CCJZ chunk descriptors may carry conservative `bounds` and `entityIds` for visible-region reads. Missing bounds require loading the chunk; this locator metadata is not a second scene or hierarchy authority. Native saves may stream unchanged entries from the previous archive by descriptor/hash, but still verify and atomically replace a newly written archive. Browser implementations support Zip64 and reject offsets or sizes beyond JavaScript's safe-integer range.
