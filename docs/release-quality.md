...
| GUI test platform | Implementation in progress / 22 registered scenarios cover production pointer, keyboard, native save/open, homogeneous and mixed-object editing, depth-two grouping, persisted arrow, expanded text-style, four-kind shape, eight-kind charge/electron-symbol, and three-kind paired-bracket matrices; the current 22-scenario candidate qualification is intentionally red at one passed and 21 missing | Versioned schemas/runner/qualification manifest, real Playwright and isolated Hyper-V production paths, coverage/precise exclusion-aware impact/resource gates, content-addressed candidates, guarded UIA/CDP targeting, real click/drag/text/Enter/shortcut input, independently bounded product-completion and transport-receipt windows, native-dialog save and reopen, independent saved-CCJS chemistry, exact properties, runs, and hierarchy indexes, selection/clipboard/history/locking/grouping, trace attribution, fail-then-pass blocking aggregation, root-confined evidence rehashing, complete bounded evidence bundles, and guest/host SHA-256 verification are operational; the bracket cell passes 29/29 real actions and 4/4 oracles after fixing missing bracket-group properties and invisible-root color application, while bracket labels/repeat links, video/crash bundles, the full feature/object/property/value and `0/1/2/many` matrix, deeper grouped/mixed cells, all format boundaries, complex/large/xlarge construction, environment coverage, model/fault/mutation testing, endurance, 1,000-repeat demonstration qualification, and final-installer qualification remain |
