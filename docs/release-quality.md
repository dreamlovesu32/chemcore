...
| GUI test platform | Implementation in progress / 19 registered scenarios cover production pointer, keyboard, native save/open, homogeneous and mixed-object editing, depth-two grouping, persisted arrow and expanded text-style matrices, multiline fixed-line-spacing, and existing-text edit/history/cancel; the machine qualification gate remains red because two long-tail failures are retained despite later passes | Versioned schemas/runner/qualification manifest, real Playwright and isolated Hyper-V production paths, coverage/precise exclusion-aware impact/resource gates, content-addressed candidates, guarded UIA/CDP targeting, real click/drag/text/Enter/shortcut input, accessible numeric-dialog validation and cancel paths, native-dialog save and reopen, independent saved-CCJS chemistry and exact-property/run-set oracles, selection/clipboard/history/locking/grouping, exact DOM-text transitions, phase-attributed action latency, per-action trace boundary markers, full-trace plus bounded performance summaries, fail-then-pass blocking aggregation, root-confined evidence rehashing, complete bounded evidence bundles, and guest/host SHA-256 verification are operational; the locked-group scheduling stall remains open despite a marked clean run, and video/crash bundles, the full feature/object/property/value and `0/1/2/many` matrix, deeper grouped/mixed cells, all format boundaries, complex/large/xlarge construction, environment coverage, model/fault/mutation testing, endurance, 1,000-repeat demonstration qualification, and final-installer qualification remain |
