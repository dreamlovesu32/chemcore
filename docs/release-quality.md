...
| GUI test platform | Implementation in progress / production pointer, keyboard, native save/open, homogeneous and mixed-object editing, depth-two grouping, persisted arrow and expanded text-style matrices, multiline fixed-line-spacing validation, immutable artifacts, and full-scenario performance-trace sentinels passed | Versioned schemas/runner, real Playwright and isolated Hyper-V production paths, coverage/precise exclusion-aware impact/resource gates, content-addressed candidates, guarded UIA/CDP targeting, real click/drag/text/Enter/shortcut input, accessible numeric-dialog validation and cancel paths, native-dialog save and reopen, independent saved-CCJS chemistry and exact-property/run-set oracles, selection/clipboard/history/locking/grouping, complete bounded evidence bundles, and guest/host SHA-256 verification are operational; video/crash bundles, the full feature/object/property/value and `0/1/2/many` matrix, deeper grouped/mixed cells, all format boundaries, complex/large/xlarge construction, environment coverage, model/fault/mutation testing, endurance, 1,000-repeat demonstration qualification, and final-installer qualification remain |
