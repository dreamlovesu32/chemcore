...
- 确立 GUI 测试平台与展示可靠性长期架构：测试平台留在主仓库并版本化场景、报告、覆盖、影响图和 Test ABI；隔离 VM 真实输入不占用户前台，所有 worker 合计限制为 10 逻辑处理器/20 GiB，并以内容寻址证据避免重跑未变闭包，同时覆盖每个功能/对象/属性、单/多对象、复杂/大文档、模型/故障/变异测试和展示资格。
