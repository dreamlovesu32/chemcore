...
严格的原始 338 张门禁还会读取 `benchmarks/public-cdxml/strict-pass-floor.json`。这份受版本控制的通过下限绑定一个精确门禁定义，并保存该定义下所有已验收通过图片的累计并集，因此即使误选了一份已经退化的基线报告，也不能把更早的绿图从历史中洗掉。严格门禁会先拒绝已经丢失受保护绿图的基线，并独立检查本次结果是否丢失任一受保护绿图。一次干净的严格门禁新增通过且没有任何回退后，只提升新增项：
