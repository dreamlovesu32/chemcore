...
审图集把两侧图像统一映射到 ChemDraw 参考图坐标系。源 CDXML 写有文档 `BoundingBox` 且 ChemDraw SVG 存在唯一占优的全局 `matrix(a 0 0 a tx ty)` 时，矩阵同时给出二十分之一点坐标到参考图像素的比例和文档原点；门禁把它与双方根 `viewBox` 原点直接组合，不再为了提高整页墨迹重叠而移动本来坐标正确的对象，也不拟合旋转或非等比变换。没有文档边界的坐标自由/布局相对源文件保留矩阵声明的固定比例，只搜索墨迹平移；矩阵缺失或不存在唯一占优值的参考图才进入同时搜索比例和平移的完整 `ink-overlap` 分支。分块和局部窗口固定锚定在 ChemDraw 参考坐标，历史配准不会覆盖当前证据。门禁使用 SVG 声明的可含小数 `width`/`height`，不使用浏览器分别取整的 `naturalWidth`/`naturalHeight`，避免候选图发生隐含的非等比拉伸。
